...
export default function MetalEdge(props) {
    const collectionName = `${props.name}-metaledge`;
    const [metalE, setMetalE] = useState([]);
    ...
    metalE.map((metalE, i) =>
        <tr id={css.row} key={i}>
            <th className={css.name}>{metalE.name}</th>
            <th></th>
            <th></th>
            { (edit == i)
            ...
            : <th className={css.waste}>{metalE.actual} <span className={css.wasteLabel}>{unitLabel(metalE.unit)}</span><span id={css.x}></span></th>
            }

            { (edit == i)
            ? <th><UnitDropdown newMargin={newMargin} setNewMargin={setNewMargin} /> </th>
            : <th className={css.waste}>{metalE.unit}<span className={css.wasteLabel}>unit</span></th>
            }
            { (edit == i) 
