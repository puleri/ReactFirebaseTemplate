...
export default function IceWater(props) {
    const collectionName = `${props.name}-icewater`;
    const [iceW, setIceW] = useState([]);
    ...
    iceW.map((iceW, i) =>
        <tr id={css.row} key={i}>
            <th className={css.name}>{iceW.name}</th>
            <th></th>
            <th></th>
            { (edit == i)
            ...
            : <th className={css.waste}>{iceW.actual} <span className={css.wasteLabel}>{unitLabel(iceW.unit)}</span><span id={css.x}></span></th>
            }

            { (edit == i)
            ? <th><UnitDropdown newMargin={newMargin} setNewMargin={setNewMargin} /> </th>
            : <th className={css.waste}>{iceW.unit}<span className={css.wasteLabel}>unit</span></th>
            }
            { (edit == i) 
