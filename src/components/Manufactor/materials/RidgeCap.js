...
export default function RidgeCap(props) {
    const collectionName = `${props.name}-ridge`;
    ...
    ridge.map((ridge, i) =>
        <tr id={css.row} key={i}>
            <th className={css.name}>{ridge.name}</th>
            <th></th>
            <th></th>
            { (edit == i)
            ...
            : <th className={css.waste}>{ridge.actual} <span className={css.wasteLabel}>{unitLabel(ridge.unit)}</span><span id={css.x}></span></th>
            }
