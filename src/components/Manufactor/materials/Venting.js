...
export default function Venting(props) {
    const collectionName = `${props.name}-venting`;
    ...
    useEffect(() => {
      ...
      // READ index of venting
    const ventingDbMatch = () => {
        // setIsLoading(true)
    ...
    const handleSubmit = () => {
        const db = firebase.firestore();
        const name = venting[edit].name;
        console.log(newMargin)

        var docRef = db.collection("templates").doc(props.name).collection(collectionName).doc(name);
        let wastePercent;
        let newState = [...venting];
    ...
    const ventingIndex =
    venting.map((venting, i) =>
        <tr id={css.row} key={i}>
            <th className={css.name}>{venting.name}</th>
            <th></th>
            <th></th>
            { (edit == i)
            ...
            : <th className={css.waste}>{venting.actual} <span className={css.wasteLabel}>{unitLabel(venting.unit)}</span><span id={css.x}></span></th>
            }

            { (edit == i)
            ? <th><UnitDropdown newMargin={newMargin} setNewMargin={setNewMargin} /> </th>
            : <th className={css.waste}>{venting.unit}<span className={css.wasteLabel}>unit</span></th>
            }
            { (edit == i) 
